...
def edit_book_admin(book_id, author_id):
    ...
    year_entry = create_field(1, "Рік видання:", year)
...
def show_users_management_window(parent_window):
    """
    Повноекранне вікно для перегляду та видалення користувачів системи.
    Показує користувачів з таблиці Users, а також стислу інформацію з usercol (якщо є).
    Дозволяє видаляти користувача та пов'язані записи в Readers / Librarians / Authors.
    """
    users_window = tk.Toplevel(parent_window)
    users_window.title("Управління користувачами")
    make_window_fullscreen(users_window)

    setup_keyboard_bindings(users_window)

    users_window.configure(bg="#F0F2F5")

    header = tk.Frame(users_window, bg="#1F4E79", height=80)
    header.pack(fill="x")
    header.pack_propagate(False)

    tk.Label(
        header,
        text="Управління користувачами системи",
        bg="#1F4E79",
        fg="white",
        font=("Segoe UI", 24, "bold")
    ).pack(pady=15)

    table_card = tk.Frame(users_window, bg="white", bd=1, relief="solid")
    table_card.pack(fill="both", expand=True, padx=20, pady=15)

    columns = ("ID", "Логін", "Роль", "Додаткова інформація")
    tree = ttk.Treeview(table_card, columns=columns, show="headings", height=25)
    ...
    tree.column("Логін", width=150, anchor="w")
    tree.column("Роль", width=120, anchor="center")
    tree.column("Додаткова інформація", width=600, anchor="w")
    ...
    def load_users():
        ...
        try:
            ...
            for user in users:
                ...
                if usercol:
                    try:
                        data = json.loads(usercol)
                        parts = []

                        if "requested_role" in data:
                            parts.append(f"Запитана роль: {data.get('requested_role')}")
                        if "status" in data:
                            parts.append(f"Статус: {data.get('status')}")

                        if "name" in data:
                            parts.append(f"Ім'я: {data.get('name')}")
                        if "surname" in data:
                            parts.append(f"Прізвище: {data.get('surname')}")
                        if "address" in data:
                            parts.append(f"Адреса: {data.get('address')}")
                        if "reader_type" in data:
                            parts.append(f"Тип читача: {data.get('reader_type')}")
                        if "university" in data:
                            parts.append(f"Університет: {data.get('university')}")
                        if "faculty" in data:
                            parts.append(f"Факультет: {data.get('faculty')}")
                        if "organization" in data:
                            parts.append(f"Організація: {data.get('organization')}")
                        if "reading_room_id" in data:
                            parts.append(f"Зал: {data.get('reading_room_id')}")

                        additional_info = ", ".join(parts) if parts else "Дані usercol без деталей"
                    except json.JSONDecodeError:
                        additional_info = "Помилка читання usercol"
                else:
                    additional_info = "Додаткових даних немає"

                tree.insert("", "end", values=(user_id, login, role, additional_info))

        except Exception as e:
            messagebox.showerror("Помилка", f"Помилка завантаження користувачів:\n{e}")
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    def delete_user():
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Увага", "Оберіть користувача для видалення.")
            return
        ...
        confirm = messagebox.askyesno(
            "Підтвердження",
            f"Видалити користувача '{login}' (ID: {user_id}, роль: {role})?\n\n"
            "Будуть також видалені пов'язані записи в Readers / Librarians / Authors (якщо є)."
        )
        if not confirm:
            return
        ...
        try:
            ...
            cursor.execute("DELETE FROM Readers WHERE user_id = %s", (user_id,))
            cursor.execute("DELETE FROM Librarians WHERE librarian_id = %s", (user_id,))
            cursor.execute("DELETE FROM Authors WHERE user_id = %s", (user_id,))

            cursor.execute("DELETE FROM Users WHERE user_id = %s", (user_id,))

            conn.commit()

            messagebox.showinfo("Успіх", f"Користувача '{login}' успішно видалено.")
            load_users()

        except Exception as e:
            if conn:
                conn.rollback()
            messagebox.showerror("Помилка", f"Помилка при видаленні користувача:\n{e}")
        finally:
            if cursor:
                cursor.close()
            if conn:
    ...
    buttons_frame = tk.Frame(users_window, bg="#F0F2F5")
    buttons_frame.pack(fill="x", padx=20, pady=(0, 15))

    def make_btn(text, cmd, bg_color, hover_color):
        btn = tk.Button(
            buttons_frame,
            text=text,
            command=cmd,
    ...
    make_btn(" Оновити список", load_users, "#2980B9", "#1F618D").pack(side="left", padx=5)
    make_btn(" Видалити обраного користувача", delete_user, "#C0392B", "#922B21").pack(side="left", padx=5)
    make_btn("Закрити", users_window.destroy, "#7F8C8D", "#616A6B").pack(side="right", padx=5)

    load_users()

def show_pending_requests(parent_window):
    """Показує вікно з запитами на підтвердження ролі"""
    requests_window = tk.Toplevel(parent_window)
    requests_window.title("Запити на підтвердження ролі")
    make_window_fullscreen(requests_window)
...
# ================== ВІКНО ПЕРЕГЛЯДУ ТА ВИДАЛЕННЯ КОРИСТУВАЧІВ ==================

def show_users_management_window(parent):
    """Повноцінне вікно перегляду та видалення користувачів"""

    win = tk.Toplevel(parent)
    win.title("Управління користувачами")
    make_window_fullscreen(win)
    setup_keyboard_bindings(win)

    header = tk.Frame(win, bg="#2C3E50", height=70)
    header.pack(fill="x")
    tk.Label(
        header, text="Перегляд та видалення користувачів",
        bg="#2C3E50", fg="white", font=("Segoe UI", 16, "bold")
    ).pack(pady=15)

    table_frame = tk.Frame(win, bg="#F8F9FA")
    table_frame.pack(fill="both", expand=True, padx=20, pady=20)

    columns = ("user_id", "login", "role")
    ...
    def load_users():
        ...
        cursor.execute("SELECT user_id, login, role FROM Users ORDER BY user_id")
    ...
    def delete_user():
        ...
        if not messagebox.askyesno(
            "Підтвердження",
            f"Видалити користувача ID {user_id}?"
        ):
            return
    ...
    tk.Button(
        btn_frame, text=" Видалити користувача ",
        bg="#E74C3C", fg="white", font=("Segoe UI", 11, "bold"),
        command=delete_user
    ).pack(side="left", padx=10)

    tk.Button(
        btn_frame, text=" Закрити (ESC) ",
        bg="#34495E", fg="white", font=("Segoe UI", 11, "bold"),
        command=win.destroy
    ).pack(side="left", padx=10)

def create_modern_button(parent, text, command, bg_color="#4A90E2", hover_color="#357ABD", 
                        text_color="white", width=30, font_size=11):
    """Створює сучасну кнопку з hover ефектом"""
    button = tk.Button(parent, text=text, command=command, 
                      bg=bg_color, fg=text_color, font=("Segoe UI", font_size, "bold"),
                      width=width, height=2, relief="flat", cursor="hand2",
                      activebackground=hover_color, activeforeground=text_color,
...
def show_admin_window(user_login):
    """Головне вікно адміністратора - ВИПРАВЛЕНА ВЕРСІЯ"""
    admin_window = tk.Tk() 
    admin_window.title(" Панель Адміністратора")
    make_window_fullscreen(admin_window)
    admin_window.configure(bg="#F8F9FA")
    admin_window.resizable(True, True)
    
    setup_keyboard_bindings(admin_window, is_main_window=True)
    ...
    try:
        admin_window.iconbitmap('admin_icon.ico')
    except:
        pass

    main_container = tk.Frame(admin_window, bg="#F8F9FA")
    ...
    def create_section(parent, title, icon=""):
        """
        Створює фрейм секції, але НЕ пакує його.
        Повертає (section_frame, content_frame)
        """
    ...
    book_section_frame, book_section_content = create_section(scrollable_frame, "Розподіл Книг")
    book_section_frame.pack(fill="x", padx=10, pady=10, anchor="n")

    tk.Label(book_section_content, text="Оберіть книгу для розподілу:",
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w", pady=(0, 5))
    
    tk.Label(book_section_content, text="(Інвентарний номер співпадає з ID книги)",
             font=("Segoe UI", 8), bg="white", fg="#7F8C8D").pack(anchor="w", pady=(0, 10))
    
    books = get_books()
    book_map = {f"{title} (ID: {book_id})": book_id for book_id, title in books}
    book_combo = create_styled_combobox(book_section_content, list(book_map.keys()), width=50)
    book_combo.pack(pady=(0, 15))

    tk.Label(book_section_content, text="Тип доступу до книги:",
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w", pady=(0, 5))
    
    access_combo = create_styled_combobox(book_section_content, 
                                          ["Тільки в читальній залі", "У читальній залі і вдома"], 
                                          width=35)
    access_combo.current(0)
    access_combo.pack(pady=(0, 15))

    tk.Label(book_section_content, text="Читальні зали:",
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w", pady=(0, 5))
    ...
    for room_id, room_name in rooms:
        var = tk.IntVar()
        checkbox = tk.Checkbutton(rooms_frame, text=room_name, variable=var,
                                 font=("Segoe UI", 9), bg="white", fg="#2C3E50",
                                 activebackground="white", selectcolor="#E8F4FD")
    ...
    tk.Label(shelf_frame, text="Полиця (Shelf):",
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w")
    shelf_combo = create_styled_combobox(shelf_frame, ["A", "B", "C", "D", "E"], width=5)
    ...
    tk.Label(row_frame, text="Ряд (Row):",
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w")
    row_combo = create_styled_combobox(row_frame, [str(i) for i in range(1, 11)], width=5)
    row_combo.current(0)
    row_combo.pack()
    ...
    save_btn = create_modern_button(book_section_content, "Розподілити книгу (Enter)", on_save, 
                                    bg_color="#27AE60", hover_color="#229954", width=25)
    ...
    operations_section_frame, operations_section_content = create_section(grid_container, "Основні Операції")
    ...
    create_modern_button(buttons_frame1, " Редагувати книгу", show_edit_book_window, 
                         bg_color="#3498DB", hover_color="#2980B9", width=22).pack(side="left", padx=5)
    
    create_modern_button(buttons_frame1, " Управління книгами", show_books_management_window, 
                         bg_color="#9B59B6", hover_color="#8E44AD", width=22).pack(side="left", padx=5)
    ...
    create_modern_button(buttons_frame1, "Управління книгами", 
                         lambda: show_books_management_window(admin_window),
                         bg_color="#9B59B6", hover_color="#8E44AD", width=22).pack(side="left", padx=5)

    create_modern_button(buttons_frame1, " Управління авторами", show_admin_author_management, 
                         bg_color="#16A085", hover_color="#138D75", width=22).pack(side="left", padx=5)

    stats_section_frame, stats_section_content = create_section(grid_container, "Статистика та Звіти")
    ...
    stats_buttons = [
        ...
        (" Відвідування",show_inactive_readers_admin_window,"#1ABC9C","#16A085"),
        (" Книги читача", show_admin_reader_search, "#34495E", "#2C3E50"),
    ]
    
    for text, command, bg_color, hover_color in stats_buttons:
        create_modern_button(stats_section_content, text, command, 
                             bg_color=bg_color, hover_color=hover_color, width=35).pack(pady=3)
    
    buttons_frame2 = tk.Frame(stats_section_content, bg="white")
    buttons_frame2.pack(fill='x', pady=5)
  
    search_section_frame, search_section_content = create_section(grid_container, "Пошук та Аналітика")
    ...
    for text, command, bg_color, hover_color in search_buttons:
        create_modern_button(search_section_content, text, command, 
                             bg_color=bg_color, hover_color=hover_color, width=35).pack(pady=3)

    users_section_frame, users_section_content = create_section(grid_container, "Управління Користувачами")
    users_section_frame.grid(row=1, column=1, padx=10, pady=10, sticky="nsew")
    
    create_modern_button(users_section_content, " Додати користувача", show_add_user_window,
                         bg_color="#9B59B6", hover_color="#8E44AD", width=35).pack(pady=3)
    
    create_modern_button(users_section_content, "Запити на підтвердження", lambda: show_pending_requests(admin_window),
                         bg_color="#E74C3C", hover_color="#C0392B", width=35).pack(pady=3)
    create_modern_button(users_section_content, "Пеегляд та видалення користувачів", lambda:show_users_management_window (admin_window),
                         bg_color="#0BBE61", hover_color="#2BC067", width=35).pack(pady=3)


    sql_section_frame, sql_section_content = create_section(grid_container, "SQL Query Editor", "")
    sql_section_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")
    sql_code = "SELECT * FROM librarysystem.readers;"

    label = tk.Label(sql_section_content, 
                 text=sql_code,
                 font=("Consolas", 10),
                 bg="white",
                 fg="black")
    ...
    tk.Label(sql_section_content, text="Введіть SQL запит:", 
             font=("Segoe UI", 10, "bold"), bg="white", fg="#2C3E50").pack(anchor="w", pady=(0, 5))
    
    sql_text = tk.Text(sql_section_content, height=8, width=70, font=("Consolas", 10), 
                       wrap=tk.WORD, bg="#1E1E1E", fg="#FFFFFF", insertbackground="white")
    ...
    def execute_sql():
        """Виконати SQL запит та показати результати"""
        ...
        try:
            ...
            if sql_query.strip().upper().startswith('SELECT'):
                ...
                messagebox.showinfo("Успіх", 
                                  ...
                                  f"Змінено рядків: {affected_rows}")
    ...
    def clear_sql():
        """Очистити текстове поле SQL"""
        sql_text.delete("1.0", tk.END)
    
    def show_sql_results(query, columns, results):
        """Показати результати SQL запиту у новому вікні"""
        results_window = tk.Toplevel(admin_window)
        ...
        tk.Label(header_frame, text="Результати SQL запиту", 
                 font=("Arial", 14, "bold"), bg="#2C3E50", fg="white").pack(pady=10)
        
        query_frame = tk.Frame(results_window, bg="#F8F9FA")
        query_frame.pack(fill="x", padx=10, pady=5)
        
        tk.Label(query_frame, text="Запит:", font=("Arial", 10, "bold"), 
                 bg="#F8F9FA").pack(anchor="w")
        
        query_text = tk.Text(query_frame, height=3, width=80, font=("Consolas", 9),
        ...
        tk.Label(stats_frame, 
                 ...
                 font=("Arial", 10, "bold"), bg="#F8F9FA").pack()
        ...
        export_btn = tk.Button(results_window, text=" Експортувати в CSV", 
                               command=export_results, bg="green", fg="white",
                               font=("Arial", 10))
        export_btn.pack(pady=10)

        close_btn = tk.Button(results_window, text="Закрити (ESC)", command=results_window.destroy,
                              bg="red", fg="white", font=("Arial", 10))
        close_btn.pack(pady=5)
    
    create_modern_button(sql_buttons_frame, " Виконати запит (Enter)", execute_sql,
                         bg_color="#27AE60", hover_color="#229954", width=20).pack(side="left", padx=5)
    
    create_modern_button(sql_buttons_frame, " Очистити", clear_sql,
                         bg_color="#E74C3C", hover_color="#C0392B", width=15).pack(side="left", padx=5)
    ...
    exit_btn = create_modern_button(exit_frame, " Вийти з системи ", logout, 
                                    bg_color="#E74C3C", hover_color="#C0392B", width=25, font_size=12)
